...
class TableWindowingInput implements WindowingInput
{
	...
	private boolean nextSplit() throws IOException
	{
		if ( currSplitNum < iSplits.length)
		{
			...
			rdr = (org.apache.hadoop.mapred.RecordReader<Writable, Writable>) iFmt.getRecordReader(curriSplit, jconf, Reporter.NULL);
			if ( key == null )
			{
				key = rdr.createKey();
	...
	private void setupSerDe(Configuration conf, Properties serDeProps) throws IOException
	{
		try
		{
			serDe = (SerDe) Class.forName(conf.get(INPUT_SERDE_CLASS)).newInstance();
